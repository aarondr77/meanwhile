...
The mark: two abstract figures side by side, each one continuous gestural line (or a couple of lines) that reads as a figure — a leaning loop, a spiral that opens, a seed shape with a dot for a head. The two figures share a rhythm without being identical twins: one may lean, coil or open a little differently. They are two people sitting beside each other, so keep them close and roughly the same size, and give each exactly one small filled dot (a head or a resting point).

Feel: peaceful, intimate, hand-drawn. Lines like a soft brush pen — even weight, moderately thick, every end and join rounded. Slightly imperfect and warm: let curves wobble a touch, let a loop not quite close, avoid perfect circles, symmetry, grids or anything that looks vector-precise. No frame, no background, no shading, no gradients, no text.
...
- Strokes: stroke-width between 3 and 4.5, stroke-linecap="round", stroke-linejoin="round". Dots are circles with r between 2.5 and 4.
